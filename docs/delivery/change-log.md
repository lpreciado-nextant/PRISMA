...
| Change | Status | Notes / next step |
|---|---|---|
| Specialization Area from 1:N lookup to native N:N (`nx_Solution_nx_SpecializationArea_nx_SpecializationArea`) | Done in Dataverse · In code | The old lookup `nx_solution.nx_specializationarea` was **deleted**, so the live connected app fails until the fix is deployed. The fix is on branch `juli` (commit `cc62794`). Next: build and test the backend, deploy the plugins **and** publish the connected app together, add the N:N to `PRISMA_Dev` (it only sits in Default), and tag "Budget Management Solution" with an area |

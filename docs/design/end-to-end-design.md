...
3. **What & why** — Exactly two fields: **What it does** for actions/results and **Business value** for the problem and benefit. Optional AI writing assistance is deferred; contributors may paste existing approved wording unchanged.
...
Following explicit user approval, both runnable submission forms automatically attempt local compression for MP4/WebM videos from 200 MiB through the existing 500 MiB input limit. Smaller files are unchanged. Only a smaller validated output replaces the selected file; failures require an explicit original-file choice or cancellation before upload. No original file is overwritten. Progress/cancellation and wizard locking are shared; the single-file presentation excludes the encoder and offers original-file fallback. Local Play worker loading and a full-length production-browser encode passed; the latter took 24m13s for five minutes of 500 MiB synthetic video, a significant usability trade-off. Packaged-host validation and GPL distribution review remain release gates. Details and verification limits are owned by [demo assets](../workflows/demo-assets.md#automatic-video-preparation).

Connected regular MP4 playback now uses caller-authorized Dataverse ranges and bounded browser buffering, including forward/backward seek. Unsupported codecs, fragmented MP4s and additional tracks offer explicit protected full-file playback; WebM retains the previous full-download path. New connected video uploads carry a private SHA-256 identity and may resume only after reopening and reselecting the exact upload file within the two-hour session deadline. Compressed upload bytes can be kept separately for resume; they are not silently reconstructed from the original. See [transfer protocol and acceptance](../workflows/demo-assets.md#protected-playback-and-resume). The local PoC has no network-upload resume protocol.

Full-file MP4 playback extracts supported `tx3g` plain-text captions into selectable browser subtitle tracks, preserving cue text/timing but not authored styling. Unsupported formats report a download fallback. Caption elements and object URLs are removed on access failure or viewer unmount. The paused-upload editor exposes the existing confirmed reopen action directly. Same-account acceptance does not establish another user's effective permissions.

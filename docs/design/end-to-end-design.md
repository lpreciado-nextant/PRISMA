...
**Status:** Agreed design; shared PoC presentation, caption saves and connected linked assets implemented; full acceptance, least-privilege verification and connected publication pending
...
Production save/submit/review operations use synchronous Dataverse Custom APIs and plug-ins with caller authorization, state validation and optimistic concurrency. Contributors cannot write protected review/publication fields directly; only an explicit PRISMA Librarian role permits approval. These handlers are now deployed in PRISMA_Dev, not SPA API routes. Owner submit/withdraw is verified; successful librarian return/approval and publication-sharing acceptance remain blocked by manual assignments and least-privilege testing. See [ADR-0008](../architecture/decisions/adr-0008-controlled-submission-transitions.md).
...
| Media | Both forms render the same PoC-baseline stacked sections, thumbnail, captioned screenshot tiles, upload areas, format picker and attachment rows. Connected upload sequencing, progress, protected previews/downloads and themed removal confirmation remain. Captions live in wizard memory across Back navigation and save through Save draft & close or Continue, with no separate caption-save button. Upload/removal flushes pending captions first. Confirmed caption versions feed subsequent core/graph saves; uncertain responses retain edits and require reopen. Finalized metadata saves preserve unfinished uploads so recovery/removal remains possible. Metadata supports order, but no reorder UI exists in either submission form. Actual document download delivery still needs browser acceptance. |
| Review and My submissions | Shared queue/card views and the PoC's two-column review action panel, feedback copy and approval styling. Connected submission cards now hydrate saved technology chips alongside contributor names and thumbnails. Server-authorized Review navigation, owner/date/media counts, independent clearance and real transitions remain. Owner-only deletion uses exact-version checks and child/media cleanup. Privileged Draft cleanup and browser Cancel/confirm/refresh passed earlier; other states have policy tests, not least-privilege acceptance. Successful librarian review remains gated on manual assignments. |
...
The source-level parity pass adds seven rendered UI/adapter-wiring checks (`npm run test:ui`) and regressions for connected card tags, named incomplete drafts and caption version chaining. On 2026-09-22 authenticated Local Play verified a temporary draft's protected PNG and HTML uploads, caption Save draft/reopen, caption retention across Back with a simultaneous summary edit, caption flush before attachment upload, and Continue to final review with fresh safety confirmation still required. A concurrent server update rejected the stale caption save, retained the on-screen edit and blocked retries until reopen. The temporary draft and media were deleted; the three original drafts were unchanged. Checks used the privileged owner, not least-privilege identities. Remaining adapter-visible differences include Dataverse checkpoints on Continue, final safety reconfirmation, withdrawal before editing non-Drafts, and additional Use case/Projects fields. Connected screenshot enlargement and protected download actions are retained. These checks do not establish successful librarian approval, non-admin access or full asset parity.

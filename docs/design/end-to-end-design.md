...
**Status:** Agreed submission revision implemented in the local mock [`app/`](../../app/README.md); production integration pending
...
**Solution detail** is the CSM's briefing document: actions, business value, builders, total effort, context, tags and media. Per-person effort and client identity are internal-only. Present mode uses only the authored anonymous context, builder names and aggregate hours. Direct hours are reported effort; calendar hours are capacity-based, not elapsed deployment time. Client-demo hours carry a warning that production delivery may take longer.
...
Effort Mode follows maturity: Direct for ideas/prototypes, Calendar for client demos/production. Direct Hours is a nonnegative two-decimal input; dates/allocation are not required in Direct mode. Calendar fields retain the existing calculation below. Switching maturity preserves draft inputs but validates and totals only the active mode.

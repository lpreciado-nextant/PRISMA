...
**Status:** Agreed design; PoC-baseline form, media and review presentation shared; full parity acceptance, least-privilege verification and connected publication pending
...
| Media | Both forms render the same PoC-baseline stacked sections, thumbnail, captioned screenshot tiles, upload areas, format picker and attachment rows. Connected upload sequencing, progress, protected previews/downloads and themed removal confirmation remain. Pending captions/uncertain media block wizard navigation without falsely displaying an active save. Dedicated thumbnail and Draft-only caption metadata passed earlier privileged live checks. Metadata supports order, but no reorder UI exists in either submission form. Actual document download delivery still needs browser acceptance. |
...
| Detail and viewer | Shared detail panels and viewer frame; caller-readable contact, effort provenance and internal library notes are projected. Present mode omits email, per-person effort, projects and notes. Protected image/HTML viewing works; document actions use the SDK. Seeded PoC URL/Power Apps/Power BI/desktop stand-ins have no connected authoring/viewer contract yet; neither PoC submission form authors those variants. |
...
The source-level parity pass adds seven rendered UI/adapter-wiring checks (`npm run test:ui`) and regressions for connected card tags and named incomplete drafts. Browser checks cover the actual PoC and authenticated connected identity steps, plus isolated shared media/review fixtures without live writes. These do not prove live upload, caption-save, approval or permission acceptance after the refactor. Remaining adapter-visible differences include Dataverse checkpoints on Continue, explicit caption saves, final safety reconfirmation, withdrawal before editing non-Drafts, and additional Use case/Projects fields. Connected screenshot enlargement and protected download actions are retained. None of these differences or the outstanding asset variants should be hidden by declaring full parity.

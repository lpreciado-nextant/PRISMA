...
| Business Calendar | Lookup → `nx_businesscalendar` | Conditional | Required in Calendar mode; automatically assigned US version covering the range |

Alternate key: `(Solution, Built By)` prevents duplicate people. Require at least one complete contributor. Validate only the active mode: direct hours or calendar dates/allocation. Preserve inactive draft inputs on maturity changes, but never total them. Apply the same rules to production writes, not just the UI.

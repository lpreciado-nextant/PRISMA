...
export function PublishedView({ solution, present, assetId }: { solution: Solution; present: boolean; assetId?: string }) {
  ...
  if (assetId) return asset ? <MediaPreview item={asset} viewerTitle={solution.name} onClose={() => navigate(`/s/${solution.id}`)} /> : <section className="mx-auto max-w-[1340px] px-4 py-6"><p role="alert" className="mb-4">Asset unavailable.</p><button className={button} onClick={() => navigate(`/s/${solution.id}`)}><Icon name="chevronLeft" />Back to solution</button></section>;
